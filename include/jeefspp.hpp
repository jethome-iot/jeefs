...
namespace jeefs {
    ...
    public:
        ...
        std::optional<std::vector<uint8_t>> readFile(const std::string &filename) {
            ...
            int16_t n = EEPROM_ReadFile(desc_, filename.c_str(), buf.data(), static_cast<uint16_t>(buf.size()));
            if (n <= 0) {
                last_error_ = n;
                return std::nullopt;
            }
        ...
        int16_t addFile(const std::string &filename, const std::vector<uint8_t> &data) {
            ...
            return remember(EEPROM_AddFile(desc_, filename.c_str(), data.data(), static_cast<uint16_t>(data.size())));
        }
        ...
        int16_t writeFile(const std::string &filename, const std::vector<uint8_t> &data) {
            ...
            if (data.size() > INT16_MAX)
                return remember(BUFFERNOTVALID);
            return remember(EEPROM_WriteFile(desc_, filename.c_str(), data.data(), static_cast<uint16_t>(data.size())));
        }
